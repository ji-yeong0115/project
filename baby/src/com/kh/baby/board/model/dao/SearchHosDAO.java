...
public class SearchHosDAO {
	


	/** 검색 내용이 포함 게시글 수 조회 DAO
	 * @param conn
	 * @param boardType
	 * @param condition
	 * @return listCount
	...
	public int getSearchCount(Connection conn, int boardType, String condition) throws Exception{
		...
		String query = "SELECT COUNT(*) FROM V_HOS_LIST WHERE BOARD_TYPE = ? "
	 ...
	 * @throws Exception
	 */
	public List<Board> selctSearch(Connection conn, Page pInfo, String condition) throws Exception{
		...
		String query = "SELECT * FROM (SELECT ROWNUM RNUM, V.* FROM "
				+ "(SELECT * FROM V_HOS_LIST WHERE BOARD_TYPE = ? AND BOARD_STATUS ='Y' AND "
