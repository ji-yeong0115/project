...
<%
	...
	List<AdminPage> list = (List<AdminPage>)request.getAttribute("reportBoard");
  ...
  <%@ include file="/WEB-INF/views/common/aside.jsp" %>  
	        	...
	        	<table class="table table-hover table-striped" id="list-table">
	                ...
	                <tbody>
	                	<%
	                		if(list.isEmpty()) {
	                	%>
	                		<tr><td colspan="8">존재하는 게시글이 없습니다.</td></tr>
	                	<%
	                		} else{
	                	%>
	                		<%
	                		...
	                		%>
	                			<tr id="tr">
	                				...
	                				<th><%=adminpage.getCreateDate() %></th>
	                				<td id="reason"><%=adminpage.getReportReason()%></td>
	                				<td>	
		                				<button type="button" class="btn btn-primary btn-sm" id="handle1">삭제</button>          				
		                				<button type="button" class="btn btn-primary btn-sm" id="handle2">유지</button>  
	                				</td>
	                			</tr>	                			
	                   		<%} %>
	                	<%} %>
	        ...
	        <div style="clear:both">
	        	<ul class = "pagination">
	        	 <%if(currentPage > 10) { %>
	        	 	<!-- 맨 처음 페이지로 이동[<<] -->	
	        	 	<li>
	        	 	 <a class="page-link" href="<%=request.getContextPath()%>/adminPage/reportBoard.do?cp=1">&lt;&lt;</a>
	        	 	</li>
	        	 	        	 
	        	 	<!-- 이전 페이지로 이동[<] -->	   
	        	 	<li>
	        	 	 <a class="page-link" href="<%=request.getContextPath()%>/adminPage/reportBoard.do?cp=<%=prev%>">&lt;</a>
	        	 	</li>
	        	 <%} %>
        	 	...
        	 	<% for(int p=startPage; p<=endPage ; p++) {%>
        	 	 ...
        	 	 <%}else{ %>
        	 	  <li>
        	 	   <a class="page-link" href="<%=request.getContextPath()%>/adminPage/reportBoard.do?cp=<%=p%>"><%=p %></a>
        	 	  </li>
        	 	 <%} %>
        	 	<%} %>
        	 	...
        	 	<%if(next < maxPage){ %>
        	 	 <!-- 다음 페이징바[>] -->
        	 	 <li>
        	 	  <a class="page-link" href="<%=request.getContextPath()%>/adminPage/reportBoard.do?cp=<%=next%>">&gt;</a>
        	 	 </li>
        	 	 
        	 	 <!-- 마지막 페이지로 이동[>>] -->
       	 		 <li>
        	 	  <a class="page-link" href="<%=request.getContextPath()%>/adminPage/reportBoard.do?cp=<%=maxPage%>">&gt;&gt;</a>
        	 	 </li>	        	 	 
        	 	<%} }%>  
	        	</ul>
	        </div>
	...
	$("#handle1").on("click",function(){
		...
		if(confirm("정말 삭제하시겠습니까?")){
			location.href = "deleteReport.do?no="+boardNo;
			}
		});
	...
	$("#handle2").on("click",function(){
		...
		location.href = "returnReport.do?no="+boardNo;
		});
	...
	$("#list-table td:nth-of-type(3)").on("click",function(){
		...
		location.href = "reportView.do?reason="+reason+"&no="+boardNo;
		<%-- "<%=request.getContextPath()%>/adminPage/reportView.do?no="+boardNo; --%>
