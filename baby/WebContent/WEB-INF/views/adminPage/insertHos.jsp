...
<form action="<%=request.getContextPath()%>/adminPage/insertHos.do?no=<%=requestNo%>&content=<%=content%>" method="post" onsubmit="return validate();">
	<table>
		<tr>
		...
		<tr>
			<th>제목 </th>
			<td style="width: 94%;"><input type="text" id="title" name="title" class="form-control" value="<%=name%>"></td>
		</tr>
	           	
	    <tr>
	       <th>영업시간 </th>
		    <td>
		       <input type="text" name="hosTime" class="form-control" value="<%=time%>">
		    </td>
	    </tr>
	    <tr>
	       <th>병원주소 </th>
		    <td>
		       <input type="text" name="hosAddress" class="form-control" id="address" value="<%=address%>">
		    </td>
	    </tr>
	    <tr>
	       <th>야간진료 여부 </th>
