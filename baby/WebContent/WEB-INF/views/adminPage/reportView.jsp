...
<% 
	AdminPage adminpage = (AdminPage)request.getAttribute("reportView");
...
<body>
	...
	<div class="container">
				<!-- Category -->
				<h6 class="mt-4" align=left>카테고리 : [<%=adminpage.getCategoryName()%>]</h6>
				<hr>
				<!-- Title -->
				<h3 class="mt-4" align=left><%=adminpage.getBoardTitle()%></h3>
				<hr>
				<!-- Writer -->
               ...
               <% if(fList != null){ %>
                ...
                <% } %>
				<!-- Content -->
				<div id="board_content"><%= adminpage.getBoardContent() %></div>
				<div id="report_reason"><%=reason%></div>
				<hr>
				<div class="float-right">
				<button type="button" class="btn btn-outline-primary" id="deleteBtn">삭제</button>
				<button type="button" class="btn btn-outline-primary" onclick="location.href ='reportBoard.do'">목록으로</button>
				</div>
			</div>
		</div>
	...
	<script>
	$("#board_content").css("text-decoration","line-through");
