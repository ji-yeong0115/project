...
%>
	...
	function selectReplyList(boardNo){
		var url = "<%=request.getContextPath()%>/communityReply/freeReplyList.do";
		console.log(boardNo);
	...
	$("#addReply").on("click", function(){
			...
			if(replyContent.trim().length == 0){
				...
				var url = "<%=request.getContextPath()%>/communityReply/freeInsertReply.do";
	...
	function deleteReply(replyNo){
		...
		if(confirm("해당 댓글을 삭제하시겠습니까?")){
			...
			var url = "<%=request.getContextPath()%>/communityReply/freeDeleteReply.do";
			var boardNo = <%=tempBoard.getBoardNo()%>
	...
	function updateReply(e,replyNo) {
		   var url = '<%=request.getContextPath()%>/communityReply/freeUpdateReply.do';
			var boardNo = <%=tempBoard.getBoardNo()%>
